  ...
  if (!hook_toball1)
  {
   ...
   RestTime=Time;
	...
	while (RestTime>ZERO)
	{
	   ...
	   //For all the balls find closest intersection between objects and planes
   	   for (int i=0;i<NR_BALLS;i++)
	   {
		      //compute new position and distance
		      OldPos[i]= sphere_poz[i];
			  sphere_speed[i] = TVector(xspeed, yspeed, zspeed);
			  //TVector::unit(sphere_speed[i],uveloc);
			  sphere_poz[i]=sphere_poz[i]+sphere_speed[i]*RestTime;
			 // rt2=OldPos[i].dist(sphere_poz[i]);
	   }
	}
	*/
}
