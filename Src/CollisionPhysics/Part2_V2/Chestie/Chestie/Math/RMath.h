...
public:
	...
	inline static double roundf(double x) { return floor(x + 0.5); }
	inline static float truncf(float x) { return ((float)((int)x));}
	inline static float expm1f(float x) { return exp(x)-1.f; }
	inline static float exp2f(float x){ return powf(2,x); }
